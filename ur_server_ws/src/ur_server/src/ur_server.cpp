...
RTDEControlInterface rtde_control("192.168.100.4");//("127.0.0.1");//
RTDEReceiveInterface rtde_receive("192.168.100.4");//("127.0.0.1");//
...
public:
  ...
  bool make_move(bool success, std::vector<std::vector<double>> this_path){
      ...
      for(int i = 0; i < this_path.size(); i++){
          ...
          std::cout << "DEBUGDEBUF AAAAA   ";
          for(int j = 0; j < 6 ; j++){
              std::cout << this_path[i][j] << "  ";
          }
          std::cout << std::endl;
  ...
  bool full_task(bool success, int goalint){
      ...
      std::vector<std::vector<double>> full_path(readfile(current_path));

      // 3) Make safety check: /////////
     // success = safety_check(full_path);

      // 4) Move robot to next position(can be preempted):
      success = make_move(success, full_path);

      // 5) Get and print resulting pose:
      print_result();
      }

      if (success == true && current_path == 9){
          finish_task = true;
      }

      last_move = current_path;

      std::cout << "FINISHED EXECUTING GOAL PATH:  " << current_path << std::endl;
      return(success);
  }
  ...
  void execute(const ur_server::urGoalConstPtr &goal)
  {
   ...
   // rtde_control.servoStop();
    ...
    // 2) Read the path file: //////////////////////////////////////
    std::vector<std::vector<double>> full_path(readfile(0));
    ...
    // 2) Read the path file: //////////////////////////////////////
    std::vector<std::vector<double>> full_path2(readfile(2));

    // 3) Make safety check: /////////
   // success = safety_check(full_path2);

    // 4) Move robot to next position(can be preempted):
    make_move(success, full_path2);

    // 5) Get and print resulting pose:
    print_result();

  /*  while (!finish_task && success == true){
        if (as_.isPreemptRequested() || !ros::ok() || success == false)
        {
          ROS_INFO("%s: Preempted", action_name_.c_str());
          // set the action state to preempted
     ...
     // rtde_control.servoJ({0.000144056, -1.5708, 3.32038e-05, -1.57255, 0.000772953, 8.05855e-05},  0.2,  0.2, 0.5, 0.2, 300);
     // std::this_thread::sleep_for(std::chrono::milliseconds(1500));
     // rtde_control.servoStop();
      full_task(success, goalint);
        }
    }*/
